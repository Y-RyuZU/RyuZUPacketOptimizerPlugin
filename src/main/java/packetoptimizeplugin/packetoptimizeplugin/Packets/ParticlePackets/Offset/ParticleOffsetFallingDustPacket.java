...
public class ParticleOffsetFallingDustPacket extends ParticleOffsetPacket {
    ...
    public boolean isSimilar(int type, int count, float speed, int blockid, int data) {
        return this.type == type && this.count == count && this.speed == speed && this.data == data && this.blockid == blockid && 21 + this.x.size() * 48 + 48 < 1024;
    }
    ...
    public static ParticleOffsetFallingDustPacket decode(byte[] byteArray) {
        ByteBuf buf = Unpooled.wrappedBuffer(byteArray);
        //IDチェック
        byte ID = buf.readByte();
        if (ID != ParticleOffsetFallingDustPacket.ID) throw new UnsupportedOperationException();
        //載せられたデータの読み取り
